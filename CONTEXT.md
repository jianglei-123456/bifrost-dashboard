...
Bifrost 的家庭媒体库管理端 Web 应用（Vue Dashboard），消费 bifrost-core 提供的管理 REST（`/api/**`）。领域词（库根 / 音乐目录 / 图书目录 / 曲目 / 艺术家 / 专辑 / 扫描 / 指纹 / 缺失文件 / 封面 / 收藏 / 评分 / 索引分组 / 管理 REST）以 bifrost-core 的 CONTEXT.md 为准；本文件只收录管理端独有的术语。后端工程位于本仓库的**兄弟目录** `../bifrost-core`（相对本目录，勿记绝对路径）。
